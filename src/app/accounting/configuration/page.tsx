...
export default function ConfigurationPage() {
  ...
  const loadCentralizedConfigs = async () => {
    try {
      ...
      if (result.success && result.data) {
        // La API actual devuelve configuración de empresa, no configuraciones de mapeo
        // Por ahora, usar array vacío hasta implementar API correcta
        setCentralizedConfigs([]);
        console.log('✅ Configuración centralizada disponible, pero no hay mapeos configurados aún');
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {/* Configuración de Empresa/Remuneraciones */}
        <Card className="bg-white/90 backdrop-blur-sm border-2 border-green-100 hover:border-green-200 transition-colors">
          ...
          <CardContent>
            {loadingCompanySettings ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900 font-semibold">{companySettings.company_name}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">RUT</label>
                    <p className="text-gray-900 font-mono">{companySettings.company_rut}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">Email</label>
                    <p className="text-gray-900">{companySettings.email}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700">Teléfono</label>
                    <p className="text-gray-900">{companySettings.phone}</p>
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Dirección</label>
                  <p className="text-gray-900">{companySettings.address}</p>
                </div>
                <div className="border-t pt-4">
                  <h4 className="text-sm font-medium text-gray-700 mb-2">Configuración Payroll</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      <span className="text-gray-600">Horas semanales:</span>
                      <span className="ml-2 font-medium">{companySettings.default_working_hours}h</span>
                    </div>
                    <div>
                      <span className="text-gray-600">AFP por defecto:</span>
                      <span className="ml-2 font-medium">{companySettings.afp_default}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Salud por defecto:</span>
                      <span className="ml-2 font-medium">{companySettings.health_default}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-lg font-medium mb-2">No hay configuración de empresa</p>
