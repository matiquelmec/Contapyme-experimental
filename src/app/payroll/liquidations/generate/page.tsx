...
export default function GenerateLiquidationPage() {
  ...
  const editLiquidationId = searchParams.get('edit');
  const employeeRut = searchParams.get('employee_id');
  ...
  // ✅ NUEVO: Manejar modo edición - preseleccionar empleado automáticamente
  useEffect(() => {
    if (isEditMode && employeeRut && employees.length > 0 && !selectedEmployeeId) {
      ...
      const employeeFound = employees.find(emp => emp.rut === employeeRut);
      ...
      if (employeeFound) {
        ...
        setSelectedEmployeeId(employeeFound.id);

        // Si hay parámetros de período, aplicarlos también
        if (searchParams.get('period_year') && searchParams.get('period_month')) {
          const periodYear = parseInt(searchParams.get('period_year'));
          const periodMonth = parseInt(searchParams.get('period_month'));

          console.log('📅 APLICANDO PERÍODO DE EDICIÓN:', { periodYear, periodMonth });
  ...
  }, [isEditMode, employeeRut, employees, selectedEmployeeId, searchParams]);

  // ✅ NUEVO: Cargar datos de liquidación existente para edición
  useEffect(() => {
    const loadExistingLiquidation = async () => {
      if (isEditMode && editLiquidationId && selectedEmployeeId) {
        console.log('📋 CARGANDO DATOS DE LIQUIDACIÓN EXISTENTE:', editLiquidationId);

        try {
          const response = await fetch(`/api/payroll/liquidations/${editLiquidationId}?company_id=${COMPANY_ID}`);
          const data = await response.json();

          if (response.ok && data.success) {
            const liquidation = data.data;
            console.log('✅ LIQUIDACIÓN CARGADA PARA EDICIÓN:', liquidation);
            ...
            setFormData(prev => ({
              ...
              overtime_hours_qty: liquidation.overtime_hours || 0,
              ...
              cash_allowance: liquidation.cash_allowance || 0,
            ...
            if (liquidation.start_work_date) {
              setPreviredData(prev => ({
                ...prev,
                start_work_date: liquidation.start_work_date || '',
                end_work_date: liquidation.end_work_date || '',
                incorporation_workplace_amount: liquidation.incorporation_workplace_amount || 0,
                sick_leave_days: liquidation.sick_leave_days || 0,
                sick_leave_start_date: liquidation.sick_leave_start_date || '',
                sick_leave_end_date: liquidation.sick_leave_end_date || '',
                sick_leave_amount: liquidation.sick_leave_amount || 0,
                vacation_days: liquidation.vacation_days || 0,
                vacation_amount: liquidation.vacation_amount || 0,
                partial_period_reason: liquidation.partial_period_reason || '',
            ...
            console.log('✅ FORMULARIO PRE-LLENADO CON DATOS EXISTENTES');
          } else {
            console.error('❌ ERROR AL CARGAR LIQUIDACIÓN:', data.error);
            setError(`Error al cargar liquidación: ${data.error}`);
          }
        } catch (err) {
          console.error('❌ ERROR DE CONEXIÓN AL CARGAR LIQUIDACIÓN:', err);
          setError('Error de conexión al cargar datos de la liquidación');
        }
      }
    };

    loadExistingLiquidation();
  }, [isEditMode, editLiquidationId, selectedEmployeeId]);
  ...
  const handleSaveAndGenerate = async () => {
    ...
    try {
      ...
      const apiUrl = isEditMode
        ? `/api/payroll/liquidations/${editLiquidationId}?company_id=${COMPANY_ID}`
        : `/api/payroll/liquidations/save?company_id=${COMPANY_ID}`;

      const method = isEditMode ? 'PUT' : 'POST';

      console.log(`🔄 ${isEditMode ? 'ACTUALIZANDO' : 'GUARDANDO'} LIQUIDACIÓN:`, {
        isEditMode,
        method,
        apiUrl,
      ...
      if (response.ok && data.success) {
        console.log(`✅ Liquidación ${isEditMode ? 'actualizada' : 'guardada'} exitosamente:`, data);

        // Determinar URL de redirección
        let redirectUrl;
        if (isEditMode && returnUrl) {
          // Si hay URL de retorno específica, usarla
          redirectUrl = returnUrl;
        } else if (isEditMode) {
          // Si es edición sin URL específica, ir a la liquidación individual
          redirectUrl = `/payroll/liquidations/${editLiquidationId}`;
        } else {
          // Si es nueva liquidación, ir a lista general
          redirectUrl = `/payroll/liquidations?saved=true`;
        }

        console.log('🔄 REDIRIGIENDO A:', redirectUrl);
        router.push(redirectUrl);
      } else {
        console.error(`❌ Error al ${isEditMode ? 'actualizar' : 'guardar'} liquidación:`, data);
        setError(data.error || `Error al ${isEditMode ? 'actualizar' : 'guardar'} liquidación: ${data.details || 'Error desconocido'}`);
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div className="flex-1">
              ...
              <p className="text-blue-100 text-sm sm:text-base mb-6">
                {isEditMode
                  ? `Editando liquidación ID: ${editLiquidationId}`
      ...
      <div className="max-w-7xl mx-auto py-8 px-4">
        ...
        {/* ✅ NUEVO: Información de modo edición */}
        {isEditMode && (
          <Card className="mb-6 bg-yellow-50 border-yellow-200">
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="font-medium text-yellow-900 mb-1">Modo Edición Activo</div>
                  <div className="text-sm space-y-1">
                    <p className="text-yellow-700">
                      Editando liquidación <span className="font-medium">ID: {editLiquidationId}</span>
                      {employeeRut && <span> para empleado RUT: <span className="font-medium">{employeeRut}</span></span>}
                    </p>
                    <p className="text-yellow-600">
                      Los datos del formulario se cargarán automáticamente. Modifica los valores que necesites y presiona "Generar y Guardar Liquidación" para aplicar los cambios.
                    </p>
                    {returnUrl && (
                      <p className="text-yellow-600">
                        Cuando termines, volverás a: <span className="font-medium">{returnUrl}</span>
                      </p>
                    )}
                  </div>
                </div>
              ...
              >
                {saving ? (
                  ...
                  <>
                    ...
                    <span>
                      {isEditMode ? 'Actualizar Liquidación' : 'Generar y Guardar Liquidación'}
                    </span>
                  </>
                )}
              </button>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    <div className="font-medium text-blue-900 mb-1">
                      {isEditMode ? 'Después de actualizar' : 'Después de guardar'}
                    </div>
                    <p className="text-blue-700">
                      {isEditMode
                        ? 'Los cambios se guardarán y volverás a la vista de la liquidación actualizada.'
                        : 'Podrás ver, imprimir y descargar la liquidación en PDF desde la lista de liquidaciones.'
                      }
                    </p>
                  </div>
                </div>
