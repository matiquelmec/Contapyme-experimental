...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedLiquidations = liquidations?.map(liquidation => ({
      id: (liquidation as any).id,
      employee_id: (liquidation as any).employee_id,
      ...
      days_worked: (liquidation as any).days_worked || 30,
      base_salary: (liquidation as any).base_salary || 0,
      legal_gratification_art50: (liquidation as any).legal_gratification_art50 || 0,
      bonuses: (liquidation as any).bonuses || 0,
      overtime_amount: (liquidation as any).overtime_amount || 0,
      total_gross_income: (liquidation as any).total_gross_income || 0,
      total_deductions: (liquidation as any).total_deductions || 0,
      net_salary: (liquidation as any).net_salary || 0,
      ...
      status: (liquidation as any).status || 'draft',
      created_at: (liquidation as any).created_at,
      updated_at: (liquidation as any).updated_at || (liquidation as any).created_at,
    })) || [];
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (status === 'draft') {
      // Solo se puede revertir a draft desde approved, no desde paid
      const paidLiquidations = existingLiquidations.filter(liq => liq.status === 'paid');
