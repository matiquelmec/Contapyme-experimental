...
function generatePreviredLine(employee: any, liquidation: any, period: string, contractsData: any[] = []): string {
  ...
  const SIS_RATE = 0.0149; // 1.49% SIS para período octubre 2025 según Previred
  ...
  // Actualizado a tasa oficial período octubre 2025: 1.49%
  const sisAmount = Math.round(imponibleAfp * SIS_RATE); // 1.49% sobre renta imponible AFP
