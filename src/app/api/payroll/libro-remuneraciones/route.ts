...
export async function GET(request: NextRequest) {
  try {
    ...
    // 🎯 RESOLVER COMPANY_ID CORRECTO AUTOMÁTICAMENTE
    ...
    // ✅ USAR DATOS REALES DE SUPABASE CON ORDENAMIENTO MEJORADO
    let query = supabase
      .from('payroll_books')
      .select(`
        id,
      ...
      .order('period', { ascending: false })      // Primero por período (2025-10, 2025-09, 2025-08...)
      .order('generation_date', { ascending: false }); // Luego por fecha de generación
    ...
    // ✅ CORREGIDO: NO usar datos demo cuando no hay libros reales
    if (booksError) {
    ...
    // Si no hay libros, retornar lista vacía (no usar datos demo)
    if (!books || books.length === 0) {
      console.log('📋 No hay libros en la base de datos - retornando lista vacía');
      return NextResponse.json({
        success: true,
        data: [],
        source: 'database',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: liquidations, error: liquidationsError } = await supabase
      ...
      .in('status', ['approved', 'paid']); // ✅ LIQUIDACIONES APROBADAS Y PAGADAS
    ...
    // Calcular totales usando la calculadora unificada para cada liquidación
    let totalHaberes = 0;
    let totalDescuentos = 0;
    let totalLiquido = 0;

    const unifiedCalculations = liquidations.map(liq => {
      // 🎯 SOLUCIÓN ROBUSTA: Usar valores existentes si están correctos, solo recalcular si hay problemas

      // Primero intentar usar los valores ya calculados en la liquidación
      let useStoredValues = true;
      let finalHaberes = liq.total_gross_income || 0;
      let finalDescuentos = liq.total_deductions || 0;
      let finalLiquido = liq.net_salary || 0;
      ...
      // Comparar valores almacenados vs calculados
      const haberesMatch = Math.abs(finalHaberes - calculatedResult.total_haberes) <= 1;
      const descuentosMatch = Math.abs(finalDescuentos - calculatedResult.total_descuentos) <= 1;
      const liquidoMatch = Math.abs(finalLiquido - calculatedResult.total_liquido) <= 1;
      ...
      if (!haberesMatch || !descuentosMatch || !liquidoMatch) {
        ...
        // Para HABERES: Usar el almacenado si es mayor (más completo)
        if (finalHaberes > calculatedResult.total_haberes) {
          console.log(`   🎯 HABERES: Usando almacenado (${finalHaberes}) - más completo que calculado (${calculatedResult.total_haberes})`);
          // finalHaberes ya tiene el valor correcto
        } else {
          finalHaberes = calculatedResult.total_haberes;
        }

        // Para DESCUENTOS: Usar el calculado (más confiable según coherencia)
        if (!descuentosMatch) {
          console.log(`   🎯 DESCUENTOS: Usando calculado (${calculatedResult.total_descuentos}) - más confiable que almacenado (${finalDescuentos})`);
          finalDescuentos = calculatedResult.total_descuentos;
        }

        // Para LÍQUIDO: Calcular matemáticamente (haberes - descuentos)
        finalLiquido = Math.round((finalHaberes - finalDescuentos) * 100) / 100;
        console.log(`   🎯 LÍQUIDO: Calculado matemáticamente (${finalHaberes} - ${finalDescuentos} = ${finalLiquido})`);
      ...
      totalHaberes += finalHaberes;
      totalDescuentos += finalDescuentos;
      totalLiquido += finalLiquido;
    ...
    // Redondear totales para evitar errores de punto flotante
    totalHaberes = Math.round(totalHaberes * 100) / 100;
    totalDescuentos = Math.round(totalDescuentos * 100) / 100;
    totalLiquido = Math.round(totalLiquido * 100) / 100;
    
    console.log('🎯 Totales calculados con Calculadora Unificada:', {
    ...
    if (inconsistencies.length > 0) {
      console.log('⚠️ Inconsistencias detectadas:', inconsistencies.map(inc => ({
        employee: inc.employee_name,
    ...
    const bookDetails = liquidations.map(liquidation => {
      ...
      return {
        ...
        apellido_paterno: employee?.last_name?.split(' ')[0] || '',                    // Primer apellido
        apellido_materno: employee?.last_name?.split(' ')[1] || '',                    // Segundo apellido (apellido materno)
...
function generateSimpleCSVFromLiquidations(liquidations: any[], book: any): string {
  ...
  const employeeRows = liquidations.map(liquidation => {
    ...
    const horasExtras = liquidation.overtime_amount || 0;  // ✅ USAR overtime_amount para horas extras
    ...
    return [
      ...
      horasExtras.toFixed(0),    // ✅ AGREGAR HORAS EXTRAS EN POSICIÓN CORRECTA
